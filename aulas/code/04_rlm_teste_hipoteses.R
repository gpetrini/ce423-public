...
tab(data.frame(`$S_{LL}$` = nm(m$S11, 1), `$S_{KK}$` = nm(m$S22, 1), `$S_{LK}$` = nm(m$S12, 1),
    ...
    caption = "Somas de desvios dos logaritmos, no exemplo")
cat(sprintf("\nTambém: $\\text{ESS} = %s$, $\\text{TSS} = %s$, $R^2 = %s$ e $F = %s$.\n",
            nm(m$ESS, 1), nm(m$TSS, 1), nm(m$R2, 3), nm(m$F, 1)))
cat(sprintf("Com $r_{LK} = %s$, o fator $1/(1-r_{LK}^2)$ vale $%s$: a variância é essa quantidade de vezes a de regressores não correlacionados.\n",
            nm(m$r12, 2), nm(1 / (1 - m$r12^2), 1)))
...
cmp <- function(t) if (abs(t) > m$tc) ">" else "<"
eq(sprintf("|%s| %s %s \\quad\\text{e}\\quad |%s| %s %s.",
           nm(m$t1, 2), cmp(m$t1), nm(m$tc, 2), nm(m$t2, 2), cmp(m$t2), nm(m$tc, 2)))
...
cat("\\end{axis}\n\\end{tikzpicture}\n")
eq(sprintf("|t(\\hat\\beta_2)| = %s < %s = t_{%s}(%s) \\qquad\\Longrightarrow\\qquad \\text{não se rejeita } H_0: \\beta_2 = 0.",
           nm(abs(m$t2), 2), nm(m$tc, 2), nm(m$alpha / 2, 3), ni(m$gl)))
cat(sprintf("\nA elasticidade estimada do capital é $%s$, tem o sinal esperado, e ainda assim não é distinguível de zero a $%s\\%%$.\n",
            nm(m$b2, 2), ni(100 * m$alpha)))
eq(sprintf("\\hat\\beta_2 \\pm t_{%s}(%s)\\, S_{\\hat\\beta_2} = %s \\pm %s \\times %s = %s.",
           nm(m$alpha / 2, 3), ni(m$gl), nm(m$b2, 2), nm(m$tc, 2), nm(m$Sb2, 3), iv(m$ic2)))
c0 <- 1
tc0 <- (m$b1 - c0) / m$Sb1
eq(sprintf("H_0: \\beta_1 = %s \\qquad t = %s = %s, \\qquad |t| %s %s.",
           ni(c0), frac(sprintf("%s - %s", nm(m$b1, 2), ni(c0)), nm(m$Sb1, 3)),
           nm(tc0, 2), if (abs(tc0) > m$tc) ">" else "<", nm(m$tc, 2)))
cat(sprintf("\n%s. A hipótese $\\beta_1 = 0$ foi rejeitada; a hipótese $\\beta_1 = %s$ %s. A mesma estimativa responde de forma diferente a perguntas diferentes.\n",
            if (abs(tc0) > m$tc) "Rejeita-se" else "Não se rejeita", ni(c0),
            if (abs(tc0) > m$tc) "também" else "não é"))
...
cat(sprintf("Com $t = %s %s %s$, aqui %s.\n", nm(m$t2, 2),
            if (m$t2 > tcu) ">" else "<", nm(tcu, 2),
            if (m$t2 > tcu) "se rejeitaria" else "também não se rejeitaria"))
...
sn <- function(p) if (p < m$alpha) "sim" else "não"
cat(sprintf("\nRejeita-se $H_0$ quando $p < \\alpha$. Com $\\alpha = %s$: $L$ %s, $K$ %s.\n",
            nm(m$alpha, 2), sn(m$p1), sn(m$p2)))
...
tab(data.frame(
  ...
  check.names = FALSE),
    caption = "Os testes individuais e o teste conjunto")
...
eq(sprintf("\\log Y_i = %s %s\\,\\log L_i %s\\,\\log K_i + u_i, \\qquad u_i \\sim \\mathcal{N}(0;\\ %s^2)",
           nm(m$dgp$logA, 1), ns(m$dgp$b1, 1), ns(m$dgp$b2, 1), nm(m$dgp$sigma, 2)))
tab(data.frame(
  Parâmetro = c("$\\beta_0 = \\log A$", "$\\beta_1$", "$\\beta_2$", "$\\sigma$"),
  Verdadeiro = paste0("$", nm(c(m$dgp$logA, m$dgp$b1, m$dgp$b2, m$dgp$sigma), 2), "$"),
  Estimado = paste0("$", nm(c(m$b0, m$b1, m$b2, m$Se), 2), "$"),
  `$t$` = c("", nm(m$t1, 2), nm(m$t2, 2), ""),
  check.names = FALSE),
    caption = "Parâmetros do processo gerador e suas estimativas", tamanho = "tiny")
## A conclusao sai dos numeros, e nao da prosa: o coeficiente do capital e
## diferente de zero no processo gerador, e o teste nao o distinguiu de zero.
cat(sprintf("\n$\\beta_2 = %s$ no processo gerador, e o teste \\alert{não o distinguiu de zero} ($t = %s$, $p = %s$).\n",
            nm(m$dgp$b2, 2), nm(m$t2, 2), nm(m$p2, 3)))
...
cat(sprintf("\\alert{Item 4.} O modelo é conjuntamente significativo ($F = %s$, $p = %s$) com um coeficiente individualmente insignificante. Aqui $r_{LK} = %s$, e o fator de inflação $1/(1-%s) = %s$ é modesto: a causa principal é o tamanho da amostra, com apenas %s graus de liberdade.\n",
